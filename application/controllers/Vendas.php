...
class Vendas extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('ci_vendedor_model');
		$this->load->model('ci_user_model');
		$this->load->model('ci_vendas_model');
		$this->load->model('ci_filial_model');
	...
	public function newVenda()
	{
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
			...
			$planos = $this->ci_user_model->getUserList();
			$vendedor = $this->ci_vendedor_model->getUserList();
			$data = array(
	...
	{
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
			$produtos = $this->ci_user_model->getUserList();
			$vendedor = $this->ci_vendedor_model->getUserList();
			$filial = $this->ci_filial_model->getUserList();
			$data = array(
	...
	public function buscaListaVendas() {
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
			...
			$vendas = $this->ci_vendas_model->getListVendas($filial, $vendedor, $plano);
			$data['aaData'] = $vendas;
	...
	public function descricao()
	{
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
			...
			$produto = $this->ci_user_model->getAllWhere($user_id, 'ID', 'produto');
	...
	public function removeVenda()
	{
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
			...
			$this->ci_vendas_model->excluirVenda($venda);
