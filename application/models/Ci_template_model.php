...
class Ci_template_model extends CI_Model {
	...
	public function getContaConfig($conta, $nomeclatura){
		$query = $this->db->query('SELECT a.id, a.status, b.nome, b.conta, r.nome, r.ativo, m.mensagem FROM bolsa AS b
					INNER JOIN ativo AS a ON a.id_cliente = b.id
					INNER JOIN robo AS r ON r.id = a.id_robo
					LEFT JOIN mensagens AS m ON m.id_robo = r.id
					WHERE b.conta = '.$conta.' AND r.nomeclatura = "'.$nomeclatura.'"');
