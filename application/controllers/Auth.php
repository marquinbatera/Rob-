...
class Auth extends CI_Controller {
	...
	function forgot_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			$this->session->set_flashdata('mensagem', 'Email não registrado, por favor verifique com o Administrador!');
			$this->session->set_flashdata('alert', 'danger');
		...
		else
		{
			...
			// $this->load->helper('form');
			// $this->parser->parse('layoutDelivery', $data);
			$this->session->set_flashdata('mensagem', 'Email para recuperar senha enviado com sucesso!');
	...
	public function reset_password($code = NULL)
	{
		...
		if ($user)
		{
			...
			// if the code is valid then display the password reset form
				...
				//$this->_render_page('auth/reset_password', $this->data);
				$this->data['view'] = 'auth/reset_password';
				$this->data['title'] = 'Btraders';
				
				$this->load->helper('form');
				$this->easy_parser->parse('layoutDelivery', $this->data);
				return;
			}
		}
		else
	...
	public function create_user()
    {
        if ($this->ion_auth->logged_in() || $this->ion_auth->is_admin())
        {
	        ...
	        if ($this->form_validation->run() == true)
	        {
	            ...
	            $groups = array($this->input->post('tipo'));
	        }
	        // Cadastra o usuário
	        if ($this->form_validation->run() == true && $this->ion_auth->register($identity, $password, $email, $additional_data, $groups))
	        {
	            ...
	            $this->session->set_flashdata('mensagem', $this->ion_auth->messages());
	            $this->data['alert'] = $this->session->set_flashdata('alert', 'success');
	            redirect(base_url('usuario/lista'), 'refresh');
	        ...
	        else
	        {
				...
				$this->easy_parser->parse('layoutPrincipal', $dat);
    ...
    function edit_client($id=null)
    {
    	...
    	else
    	{
    		...
    		$this->load->helper('form');
    		$this->easy_parser->parse('layout', $data);
    	}
    }
    ...
    function edit_user($id)
    {
		...
		// validate form input
    	$this->form_validation->set_rules('username', $this->lang->line('edit_user_validation_fname_label'), 'required');
    	$this->form_validation->set_rules('first_name', $this->lang->line('edit_user_validation_lname_label'), 'required');
    	...
    	if (isset($_POST) && !empty($_POST))
    	{
    		...
    		if ($this->form_validation->run() === TRUE)
    		{
    			...
    			if ($this->ion_auth->logged_in() && $this->input->post('groups'))
    			{
			...
			// check to see if we are updating the user
    			...
    			{
			    	// redirect them back to the admin page if admin, or to the base url if non admin
    				$this->data['message'] = $this->session->set_flashdata('mensagem', $this->ion_auth->messages());
    				$this->data['alert'] = $this->session->set_flashdata('alert', 'danger');
