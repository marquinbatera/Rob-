<?php
class Ci_vendas_model extends CI_Model {
	...
	public function getListVendas($filial = null, $vendedor = null, $plano = null)
	{
		if(!empty($filial))
			$and .= ' AND f.id = '.$filial;
		if(!empty($vendedor))
			$and .= ' AND vo.id = '.$vendedor;
		if(!empty($plano))
			$and .= ' AND p.ID = '.$plano;
		$query = $this->db->query('SELECT va.id, p.Nome AS plano, p.Preco, vo.nome AS vendedor, f.nome AS filial FROM vendas AS va
				INNER JOIN vendedor AS vo ON vo.id = va.id_vendedor
				INNER JOIN produto AS p ON p.ID = va.id_plano
				INNER JOIN filial AS f ON f.id = vo.id_filial
				WHERE va.id IS NOT NULL '.$and. '
				GROUP BY va.id');
